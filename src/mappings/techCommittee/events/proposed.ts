...
type TechnicalCommitteeProposalStorageData = ProposalCall
...
async function getStorageData(
    ...
    let storage: TechnicalCommitteeProposalOfStorage | Instance2CollectiveProposalOfStorage =
        new TechnicalCommitteeProposalOfStorage(ctx)
    if (storage.isExists) {
        if (storage.isV9111) {
        ...
        storage = new Instance2CollectiveProposalOfStorage(ctx)
        if (storage.isExists) {
            if (storage.isV1020) {
