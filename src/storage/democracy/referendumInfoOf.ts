...
async function getStorageData(ctx: BlockContext, index: number): Promise<ReferendumStorageData | undefined> {
    ...
    if (storage.isV1090) {
        const storageData = await storage.getAsV1090(index)
        if (!storageData) return undefined

        const { __kind: status } = storageData
        if (status === 'Ongoing') {
            const { proposalHash: hash, end, delay, threshold } = (storageData as v1090.ReferendumInfo_Ongoing).value
            return {
                status,
                hash,
                end,
            ...
            const { end, approved } = storageData as v1090.ReferendumInfo_Finished
            return {
                status,
                end,
                approved,
